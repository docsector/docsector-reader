...
- 🔄 **Update Notification** — Production builds stamp a build ID (Cloudflare Pages commit SHA, `DOCSECTOR_BUILD_ID`, or a per-build timestamp) into the bundle and into an emitted `version.json` (with a `Cache-Control: no-cache` `_headers` rule); open tabs poll it while visible and on re-focus, showing a theme-colored floating **Refresh** / **Dismiss** banner when a newer deploy is live — localized via `system.update.*`, tunable or disabled via the `updates` config key
...
- 🧰 **Code Block Toolbar Override** — The metadata row (language label + copy button) follows the content by default — shown on multi-line, tabbed or breadcrumbed blocks, hidden on single-line ones — and the `toolbar` fence attribute overrides it both ways: `:toolbar="true";` opts a one-line command in, `:toolbar="false";` strips it from any block
